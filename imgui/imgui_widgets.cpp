...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (is_visible)
    {
        ...
        RenderFrame(check_bb.Min + ImVec2(0, 1), check_bb.Max + ImVec2(0, 1), GetColorU32((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), true, style.FrameRounding);
        ...
        if (mixed_value)
        {
            ...
            window->DrawList->AddRectFilled(check_bb.Min + pad + ImVec2(0, 1), check_bb.Max - pad + ImVec2(0, 1), check_col, style.FrameRounding);
        }
        else if (*v)
        {
            const float pad = ImMax(1.0f, IM_TRUNC(square_sz / 6.0f));
            window->DrawList->AddRectFilled(check_bb.Min + ImVec2(0, 1), check_bb.Max + ImVec2(0, 1), check_col, style.FrameRounding);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y * 2));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    frame_bb.Min = frame_bb.Min + ImVec2(0, 4);
    frame_bb.Max = frame_bb.Max - ImVec2(0, 4);
    // Draw frame
    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    RenderNavHighlight(frame_bb, id);
    ...
    /*grab_bb.Max = grab_bb.Max - ImVec2(0, 10);*/
    ...
    if (g.LogEnabled)
        LogSetNextTextDecoration("{", "}");
    //RenderTextClipped(frame_bb.Min, frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));
    int totalLength = strlen(value_buf) + strlen(label) + 2; // +1 for the null terminator
    char* result = new char[totalLength];
    frame_bb.Min = frame_bb.Min - ImVec2(0, 4);
    frame_bb.Max = frame_bb.Max + ImVec2(0, 4);
    // Copy the first string into result
    result[0] = ' ';
    strcpy(result, value_buf);

    // Concatenate the second string to result
    strcat(result, label);
    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x + 10, frame_bb.Min.y + style.FramePadding.y), result);
