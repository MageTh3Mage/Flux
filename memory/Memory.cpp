...
bool Memory::FixCr3()
{
	if (first) {
		...
		if (result) {
			LOG("[+] no need to patch\n"); //Doesn't need to be patched lol
			return true;
		}
	...
	NTSTATUS nt = VMMDLL_VfsReadW(this->vHandle, (LPWSTR)L"\\misc\\procinfo\\dtb.txt", bytes.get(), buffer_size - 1, &j, 0);
	//std::cout << "bytes.get(): " << static_cast<void*>(bytes.get()) << std::endl; // Pointer address
	//std::cout << "buffer_size - 1: " << buffer_size - 1 << std::endl; // Size_t value
